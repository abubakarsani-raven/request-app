...
const ictWorkflowStages = [
  ...
  { stage: 'DDICT_REVIEW', role: 'DDICT', description: 'DDICT Review' },
...
export default function ICTRequestDetailsPage() {
  ...
  const { data: request, isLoading } = useQuery<ICTRequestDetail>({
    ...
    queryFn: () => fetchJSON<ICTRequestDetail>(`/api/ict-requests/${requestId}`),
    enabled: !!requestId,
  });
  ...
  const handleFulfill = async () => {
    ...
    try {
      const res = await fetch(`/api/ict-requests/${requestId}/fulfill`, {
        method: "PUT",
      ...
      if (res.ok) {
        qc.invalidateQueries({ queryKey: ["ict-request", requestId] });
  ...
  const handleApprove = async (comment: string, isAdminApproval: boolean) => {
    ...
    try {
      const res = await fetch(`/api/ict-requests/${requestId}/approve`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ 
  ...
  const handleReject = async (reason: string) => {
    ...
    try {
      const res = await fetch(`/api/ict-requests/${requestId}/reject`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
  ...
  const handleSendBack = async (reason: string) => {
    ...
    try {
      const res = await fetch(`/api/ict-requests/${requestId}/send-back`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ reason }),
      });
  ...
  const handleNotify = async (message: string) => {
    ...
    try {
      const res = await fetch(`/api/ict-requests/${requestId}/notify-requester`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      if (res.ok) {
        ...
        const data = await res.json().catch(() => ({}));
        throw new Error(data.message || "Failed to notify requester");
      }
    } catch (err: any) {
      throw err;
    }
  ...
  // Admin can approve at any stage, SO can approve at SO_REVIEW
  const canApprove = permissions.canManageICT || 
    (user?.roles?.includes('SO') &&
    workflowStage === "SO_REVIEW" &&
    (request.status === "PENDING" || request.status === "CORRECTED"));
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-4">
        ...
        <h1 className="text-2xl font-bold">ICT Request Details</h1>
      </div>
      ...
      <Tabs defaultValue="details" className="space-y-4">
        ...
        <TabsContent value="workflow" className="space-y-4">
          <WorkflowTimeline
            stages={ictWorkflowStages.map((s) => ({
              stage: s.stage,
              role: s.role,
