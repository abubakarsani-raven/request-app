...
export function UpdateQuantityModal({ open, item, onClose }: UpdateQuantityModalProps) {
  ...
  async function handleSubmit() {
    ...
    try {
      const res = await fetch(`/api/store-inventory/${itemId}/quantity`, {
        method: "PATCH",
      ...
      if (res.ok) {
        ...
        queryClient.invalidateQueries({ queryKey: ["store-items-low-stock"] });
        queryClient.invalidateQueries({ queryKey: ["store-stock-history", itemId] });
        onClose();
      } else {
